...
export function PaginationPrevious({
  ...
  className,
  children = 'Previous',
}: React.PropsWithChildren<{ onClick?: (() => void) | null; disabled?: boolean; className?: string }>) {
  return (
    <span className={clsx(className, 'grow basis-0')}>
      <Button {...(onClick === null ? {} : { onClick })} disabled={disabled} plain aria-label="Previous page">
...
export function PaginationNext({
  ...
  return (
    <span className={clsx(className, 'flex grow basis-0 justify-end')}>
      <Button {...(onClick === null ? {} : { onClick })} disabled={disabled} plain aria-label="Next page">
        {children}
        <svg className="stroke-current" data-slot="icon" viewBox="0 0 16 16" fill="none" aria-hidden="true">
          <path
